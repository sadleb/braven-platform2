...
RSpec.describe LessonContentUtil do

  let(:aws_object) { instance_double(Aws::S3::Object) }
  let(:lesson_content) { create(:lesson_content_with_zipfile) }

  before(:each) do
    allow_any_instance_of(LessonContentUtil::AwsS3Bucket).to receive(:object).and_return(aws_object)
  ...
  describe '#launch_path' do
    context 'when valid Rise360 zipfile' do
      ...
      it 'returns launch URL' do
        allow(LessonContentUtil).to receive(:publish).and_return("https://S3-bucket-path/lessons/somekey/index.html")
        expect(aws_object).to receive(:public_url).and_return("https://S3-bucket-path/lessons/somekey/index.html")
        expect(LessonContentUtil.launch_path(lesson_content.lesson_content_zipfile.key)).to eq('/lessons/somekey/index.html')
      end
