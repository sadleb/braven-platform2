...
RSpec.describe GenerateZoomLinksJob, type: :job do
  describe '#perform' do
    ...
    it 'sends success mail if successful' do
      GenerateZoomLinksJob.perform_now(meeting_id, email, participants)
    ...
    it 'sends failure mail if something bad happens' do
      ...
      expect{ GenerateZoomLinksJob.perform_now(meeting_id, email, participants) }.to raise_error('something bad')
